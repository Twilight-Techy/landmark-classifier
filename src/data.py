...
def get_data_loaders(
    ...
    # If user passed num_workers < 0, use all CPUs
    if num_workers < 0:
        num_workers = multiprocessing.cpu_count()

    # Convert mean/std to python lists for transforms.Normalize
    mean_list = mean.tolist() if hasattr(mean, "tolist") else list(mean)
    std_list = std.tolist() if hasattr(std, "tolist") else list(std)
    ...
    # ensure at least 1 validation sample when valid_size > 0 but small
    if split == 0 and valid_size > 0:
        split = 1
...
def visualize_one_batch(data_loaders, max_n: int = 5):
    ...
    # Get class names from the train data loader (handle Subset wrappers)
    train_dataset = data_loaders["train"].dataset
    if hasattr(train_dataset, "dataset"):
        # If the DataLoader was given a Subset or similar, unwrap to get ImageFolder
        class_names = train_dataset.dataset.classes
    else:
        class_names = train_dataset.classes

    # Convert from (B, C, H, W) to (B, H, W, C), move to cpu and to numpy and clip
    images = images.cpu().permute(0, 2, 3, 1).numpy().clip(0, 1)

    # plot the images in the batch, along with the corresponding labels
    fig = plt.figure(figsize=(25, 4))
    for idx in range(min(max_n, images.shape[0])):
